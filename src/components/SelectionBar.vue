<template>
  ...
  >
    <div v-if="count > 0" class="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center rounded-2xl bg-indigo-600 px-4 py-3 shadow-lg">
      <!-- Счётчик + Сбросить -->
      ...
      <button
        @click="$emit('delete')"
        class="ml-12 flex items-center gap-x-1.5 rounded-xl bg-indigo-500 hover:bg-indigo-400 px-3 py-1.5 text-[14px] font-medium text-white transition-colors"
      >
        <Trash2Icon :size="14" />
        Удалить
      </button>
