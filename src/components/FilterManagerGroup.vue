<template>
  <div class="px-6 py-4">
    ...
    <div :class="['flex flex-col gap-y-3', searchQuery ? '' : 'mt-5']">
      <div
        v-for="mgr in displayedOptions"
        :key="mgr.name"
        class="flex items-center justify-between cursor-pointer"
        @click="toggle(mgr.name)"
      >
...
const props = defineProps({
  ...
  options: { type: Array, required: true }, // [{ name, initials, role }]
  modelValue: { type: Set, required: true },
  searchPlaceholder: { type: String, default: 'Фамилия или имя' },
})
...
const filteredOptions = computed(() => {
  const base = searchQuery.value.trim()
    ? props.options.filter(m => m.name.toLowerCase().includes(searchQuery.value.trim().toLowerCase()))
    : props.options
...
const displayedOptions = computed(() => {
  ...
  const selectedBeyond = base.slice(VISIBLE_COUNT).filter(m => props.modelValue.has(m.name))
...
const hiddenCount = computed(() => {
  const base = filteredOptions.value
  const selectedBeyond = base.slice(VISIBLE_COUNT).filter(m => props.modelValue.has(m.name)).length
...
const toggleAll = () => {
  emit('update:modelValue', allSelected.value ? new Set() : new Set(props.options.map(m => m.name)))
}
</script>
