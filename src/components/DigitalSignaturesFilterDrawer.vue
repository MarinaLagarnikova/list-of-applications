    ...
    <div class="flex-1 overflow-y-auto min-h-0">
      ...
      <div class="px-6 py-4 flex flex-col gap-y-2">
        <div class="flex items-center">
          <input id="sort-sig-new" name="sort-sig" type="radio" value="new" v-model="sortOrder" class="relative size-4 appearance-none rounded-full border border-gray-300 bg-white before:absolute before:inset-1 before:rounded-full before:bg-white not-checked:before:hidden checked:border-indigo-600 checked:bg-indigo-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 forced-colors:appearance-auto forced-colors:before:hidden" />
          <label for="sort-sig-new" class="ml-3 text-[14px] font-light text-[#18181b]">Сначала новые</label>
        </div>
        <div class="flex items-center">
          <input id="sort-sig-old" name="sort-sig" type="radio" value="old" v-model="sortOrder" class="relative size-4 appearance-none rounded-full border border-gray-300 bg-white before:absolute before:inset-1 before:rounded-full before:bg-white not-checked:before:hidden checked:border-indigo-600 checked:bg-indigo-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 forced-colors:appearance-auto forced-colors:before:hidden" />
          <label for="sort-sig-old" class="ml-3 text-[14px] font-light text-[#18181b]">Сначала старые</label>
        </div>
      </div>
      ...
      <div class="px-6 py-4">
        <div class="flex items-center justify-between mb-3">
          <span class="text-[14px] leading-[24px] font-medium text-[#18181b]">Способ выпуска</span>
          <button
            @click="selectedMethods.size > 0 ? resetSet(selectedMethods) : fillSet(selectedMethods, signMethods)"
            class="text-[14px] font-medium text-indigo-600 hover:text-indigo-700"
          >{{ selectedMethods.size > 0 ? 'Сбросить' : 'Выбрать все' }}</button>
        </div>
        <div class="flex flex-col gap-y-3">
          <div
            ...
            :key="method"
            class="flex items-center justify-between cursor-pointer"
            @click="toggleSet(selectedMethods, method)"
          >
            <span class="text-[14px] font-light text-[#18181b]">{{ method }}</span>
            <span :class="checkboxClass(selectedMethods.has(method))">
              <svg :class="['size-3 stroke-white transition-opacity', selectedMethods.has(method) ? 'opacity-100' : 'opacity-0']" viewBox="0 0 14 14" fill="none"><path d="M3 8L6 11L11 3.5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
            </span>
          </div>
        </div>
      </div>
...
// Способ выпуска
const signMethods = ['Офис', 'Онлайн']
const selectedMethods = ref(new Set())

// Статус
const statuses = ['В работе', 'На выпуске', 'Активна']
