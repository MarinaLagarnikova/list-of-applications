    ...
    <div class="flex-1 overflow-auto" @scroll="isScrolled = $event.target.scrollTop > 0">
      <div class="pt-6 pb-10">
        <table class="w-full border-separate border-spacing-0">
          <tbody>
            ...
            >
              ...
              <!-- Менеджер -->
              <td class="group-hover:bg-zinc-50 [.selected_&]:bg-zinc-50 [.selected_&]:group-hover:bg-zinc-100 align-middle pl-[22px] pr-8 py-4 whitespace-nowrap" @click.stop>
                <CatalystListbox
        ...
        <div v-if="filteredItems.length === 0 && !searchQuery.trim() && !props.filterCount" class="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p class="mt-1 text-sm font-light text-zinc-500">В этом списке пока нет пакетов документов</p>
...
const items = [
  { id: 'ПД 1150', date: '17.04', status: 'Подписан',           statusDate: '17.04', name: 'Ипотека Сбербанк — Новикова Е.Д.',      signers: 2, signedCount: 2, manager: 'Новиков Алексей',
    signersList: [
      { initials: 'НЕ', name: 'Новикова Е.Д.',  phone: '+7 931 208-05-14', signed: true },
      { initials: 'НА', name: 'Новиков А. С.',   phone: '+7 931 208-05-22', signed: true },
    ],
  },
  { id: 'ПД 1149', date: '17.04', status: 'Подписан',           statusDate: '17.04', name: 'Семейная ипотека — Орлов Д.П.',          signers: 1, signedCount: 1, manager: 'Орлов Дмитрий',
    signersList: [
      { initials: 'ОД', name: 'Орлов Д.П.',      phone: '+7 925 111-22-33', signed: true },
    ],
  },
  { id: 'ПД 1148', date: '16.04', status: 'Черновик',           statusDate: null,    name: 'Льготная ипотека — Смирнова Ю.А.',       signers: 3, signedCount: 0, manager: 'Смирнова Юлия',
    signersList: [
      { initials: 'СЮ', name: 'Смирнова Ю.А.',   phone: '+7 916 333-44-55', signed: false },
      { initials: 'ОД', name: 'Орлов Д.П.',       phone: '+7 925 111-22-33', signed: false },
      { initials: 'ЛИ', name: 'Лебедев И.Н.',     phone: '+7 903 222-33-44', signed: false },
    ],
  },
  { id: 'ПД 1147', date: '08.04', status: 'Готов к подписанию', statusDate: null,    name: 'Рефинансирование — Лебедев И.Н.',        signers: 2, signedCount: 1, manager: 'Лебедев Игорь',
    signersList: [
      { initials: 'ЛИ', name: 'Лебедев И.Н.',    phone: '+7 903 222-33-44', signed: true  },
      { initials: 'НЕ', name: 'Новикова Е.Д.',   phone: '+7 931 208-05-14', signed: false },
    ],
  },
  { id: 'ПД 1146', date: '21.01', status: 'Готов к подписанию', statusDate: null,    name: 'Ипотека ВТБ — Воронова А.С.',           signers: 2, signedCount: 0, manager: 'Воронова Анна',
    signersList: [
      { initials: 'ВА', name: 'Воронова А.С.',   phone: '+7 921 444-55-66', signed: false },
      { initials: 'СЮ', name: 'Смирнова Ю.А.',   phone: '+7 916 333-44-55', signed: false },
    ],
  },
  { id: 'ПД 1145', date: '15.01', status: 'Подписан',           statusDate: '16.01', name: 'Договор долевого участия — Могиль М.В.', signers: 4, signedCount: 4, manager: 'Новиков Алексей',
    signersList: [
      { initials: 'ММ', name: 'Могиль М.В.',     phone: '+7 901 234-56-19', signed: true },
      { initials: 'НЕ', name: 'Новикова Е.Д.',   phone: '+7 931 208-05-14', signed: true },
      { initials: 'НА', name: 'Новиков А. С.',    phone: '+7 931 208-05-22', signed: true },
      { initials: 'ОД', name: 'Орлов Д.П.',       phone: '+7 925 111-22-33', signed: true },
    ],
  ...
  { id: 'ПД 1144', date: '10.01', status: 'Черновик',           statusDate: null,    name: 'IT-ипотека — Андреев Р.Е.',             signers: 1, signedCount: 0, manager: 'Морозов Сергей',
    signersList: [
      { initials: 'АР', name: 'Андреев Р.Е.',    phone: '+7 911 555-66-77', signed: false },
    ],
  },
  { id: 'ПД 1143', date: '05.01', status: 'Готов к подписанию', statusDate: null,    name: 'Ипотека Альфа-Банк — Морозов С.В.',     signers: 3, signedCount: 2, manager: 'Смирнова Юлия',
    signersList: [
      { initials: 'МС', name: 'Морозов С.В.',    phone: '+7 910 666-77-88', signed: true  },
      { initials: 'СЮ', name: 'Смирнова Ю.А.',   phone: '+7 916 333-44-55', signed: true  },
      { initials: 'НЕ', name: 'Новикова Е.Д.',   phone: '+7 931 208-05-14', signed: false },
    ],
  },
]
